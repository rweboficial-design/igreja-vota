...
export default function TechDashboard() {
  ...
  // === grava a fase global no backend e também atualiza o store local ===
  ...
  const tabButton = (t) => ({
    ...
    marginRight: 6, cursor: 'pointer', fontWeight: 600
  });
          ...
          <div>
            ...
            <div style={{ background: '#1f2937', padding: 16, borderRadius: 8, marginBottom: 20 }}>
              ...
              {status ? (
                <>
                  <p>
                    <strong>Ministério:</strong> {status.ministry_name || '-'}<br />
                    <strong>Cargo:</strong> {status.role_name || '-'}<br />
                    <strong>Tipo de Sessão:</strong>{' '}
                    {status.voting_active ? 'Votação'
                      : status.indication_active ? 'Indicação' : 'Nenhuma ativa'}
                  </p>
                  <div style={{ marginTop: 8 }}>
                    <strong>Membros logados:</strong> {status.total_members || 0}<br />
                    <strong>Participantes que já votaram:</strong> {status.voted_count || 0}
                  </div>
                  <div style={{ background: '#374151', borderRadius: 6, overflow: 'hidden', height: 14, marginTop: 10 }}>
                    <div style={{ width: `${progressPercent}%`, height: '100%', background: '#22c55e', transition: 'width .3s ease' }} />
                  </div>
            ...
            <div style={{ background: '#1f2937', padding: 16, borderRadius: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
                <select value={selectedMin} onChange={(e) => setSelectedMin(e.target.value)} style={selectStyle}>
                  <option value="">Selecione o ministério…</option>
                  {ministries.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
                </select>
                <select value={selectedRole} onChange={(e) => setSelectedRole(e.target.value)} style={selectStyle}>
                  <option value="">Selecione o cargo…</option>
                  {roles.filter((r) => !selectedMin || r.ministry_id === selectedMin).map((r) => (
                    <option key={r.id} value={r.id}>{r.name}</option>
                  ))}
                </select>
              </div>
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <button onClick={startIndication} style={baseButton} disabled={!selectedMin || !selectedRole}>
                  Iniciar Indicação
                </button>
                <button onClick={startVoting} style={secondaryButton} disabled={!selectedMin || !selectedRole}>
                  Iniciar Votação
                </button>
            ...
            <div style={{ marginTop: 20 }}>
              <a href="/api/report" target="_blank" rel="noreferrer" style={{ color: '#3b82f6', textDecoration: 'none', fontWeight: 600 }}>
